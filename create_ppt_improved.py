...
def create_improved_rag_presentation():
    # 创建演示文稿
    prs = Presentation()
    ...
    # 优化字体大小和样式
    title.text_frame.paragraphs[0].font.size = Pt(36)  # 缩小字体
    title.text_frame.paragraphs[0].font.color.rgb = RGBColor(0, 51, 102)
    title.text_frame.paragraphs[0].font.bold = True
    
    subtitle.text_frame.paragraphs[0].font.size = Pt(20)  # 缩小字体
    ...
    p = tf.paragraphs[0]
    p.font.size = Pt(32)  # 缩小字体
    p.font.color.rgb = RGBColor(0, 51, 102)
    p.font.bold = True
    p.alignment = PP_ALIGN.CENTER
    
    # 保存PPT文件
    pptx_file = "RAG文档问答系统-企业汇报-优化版.pptx"
    prs.save(pptx_file)
    print(f"优化版PPT文件已创建: {pptx_file}")
    
    return pptx_file

if __name__ == "__main__":
    try:
        ppt_file = create_improved_rag_presentation()
        print("✅ 优化版企业汇报PPT创建成功！")
        print(f"📊 文件位置: {ppt_file}")
        print("🎯 包含10页优化版幻灯片")
        print("✨ 优化内容:")
        print("   • 字体大小优化（标题36pt，内容14-16pt）")
        print("   • 启用自动换行功能")
        print("   • 更好的布局和可读性")
